...
tissues <- c("A+TnA", "VTA+SN", "POM", "Hyp", "NC", "Rap", "LS")
samples <- samples[samples$Tissue %in% tissues, ]
...
pcaData$Tissue <- factor(pcaData$Tissue, levels = c("A+TnA", "VTA+SN", "POM", "Hyp", "NC", "Rap", "LS"))
...
p <- ggplot(pcaData, aes(PC1, PC2, fill = Tissue, shape = Phenotype)) +
    ...
    geom_point(aes(fill = Tissue), 
               ...
               stroke = .35,
               shape = 21) +
    scale_fill_manual(name = "Brain region",
                      labels = c("A+TnA", "VTA+SN", "POM", "Hyp", "NC", "Rap", "LS"),
                      values = alpha(c("coral1", "darkgoldenrod", "deepskyblue", "mediumpurple1","limegreen","mediumblue", "hotpink3"), 1)) +
...
ggsave(filename = "supplementary_figures/plots/PCA_Brain_areas.svg", plot = p, 
