...
# Read DESeq2 results for Satellite vs. Independent phenotypes
deseq2_results <- read.csv("DESeq2_gene_expression_analysis/DESeq2_results/tissue/full_results/DGE_DESeq2_satellite_vs_independent_Hyp_FR.csv")
...
genes <- deseq2_results[deseq2_results$Gene_symbol %in% c("CENPN", "SDR42E1", "PLCG2", "HSD17B2", "TUBB3"), ]

# Create ggplot object
p <- ggplot(deseq2_results, aes(x = log2FoldChange, y = log10padj, color = colors, shape = shapes, fontface = "italic")) + 
  geom_point(data = deseq2_results[deseq2_results$Gene_symbol %in% c("CENPN", "SDR42E1", "PLCG2"), ],
             aes(x = log2FoldChange, y = log10padj), 
             size = 6, color = "black", shape = 19) +
  geom_point(data = deseq2_results[deseq2_results$Gene_symbol %in% c("HSD17B2", "TUBB3"), ],
             aes(x = log2FoldChange, y = log10padj), 
  ...
  ggtitle("Satellite vs. Independent") +
  theme(
  ...
  geom_text_repel(data = deseq2_results[deseq2_results$Gene_symbol %in% "SDR42E1", ],
  ...
  geom_text_repel(data = deseq2_results[deseq2_results$Gene_symbol %in% "PLCG2", ],
                  ...
                  nudge_x = 0.75,
                  size = 8) +
  geom_point(data = deseq2_results[deseq2_results$Gene_symbol %in% c("SDR42E1", "PLCG2"), ],
             aes(x = log2FoldChange, y = log10padj), 
             size = 6, color = "black", shape = 19,) +
  geom_point(data = deseq2_results[deseq2_results$Gene_symbol %in% c("SDR42E1", "PLCG2"), ],
  ...
  geom_text_repel(data = deseq2_results[deseq2_results$Gene_symbol %in% "CENPN", ],
                  ...
                  size = 8,
                  parse = TRUE) +
  geom_point(data = deseq2_results[deseq2_results$Gene_symbol %in% "CENPN", ],
             aes(x = log2FoldChange, y = log10padj), 
             size = 6, color = "black", shape = 19) +
  geom_point(data = deseq2_results[deseq2_results$Gene_symbol %in% "CENPN", ],
...
ggsave(filename = "main_figures/plots/VolcanoPlot_Hyp_Sat_vs_Ind.svg", plot = p, 
