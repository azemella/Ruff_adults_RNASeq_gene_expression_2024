...
res <- results(dds, contrast = c("Phenotype", "Faeder", "Independent"), alpha = 0.05, parallel = TRUE)
res <- lfcShrink(dds, contrast = c("Phenotype", "Faeder", "Independent"), res = res, type = "ashr", parallel = TRUE)

# Prepare results for plotting on karyotype
res <- as.data.frame(res)
rownames(res)[rownames(res) == "CDH3"] <- "LOC106888173"
rownames(res)[rownames(res) == "DUSP22A-like"] <- "LOC106888098"
...
# Set up SVG output
svg("main_figures/plots/karyoploteR_inversion_faeder_vs_independent_zoom.svg", width = 28, height = 14)
...
top.genes <- sign.genes[sign.genes$gene_id %in% c("HSD17B2", "CENPN", "DPEP1", "LOC106888173", "LOC106888098")]
names(top.genes)[5] <- "CDH3"
names(top.genes)[4] <- "\n\nDUSP22A\n       -like"
points.top <- 0.8
...
kpAddLabels(kp, labels = expression('Log'[2]*'FC Faeder vs. Independent'), srt = 90, pos = 1, label.margin = 0.07, 
