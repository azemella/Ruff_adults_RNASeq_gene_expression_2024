...
# Read DESeq2 results for Faeder vs. Independent phenotypes
deseq2_results <- read.csv("DESeq2_gene_expression_analysis/DESeq2_results/tissue/full_results/DGE_DESeq2_faeder_vs_independent_Hyp_FR.csv")
...
genes <- deseq2_results[deseq2_results$Gene_symbol %in% c("ARY2", "HSD17B2", "CYBA", "CENPN", "CDH3"), ]

# Create ggplot object
p <- ggplot(deseq2_results, aes(x = log2FoldChange, y = log10padj, color = colors, shape = shapes, fontface = "italic")) + 
  geom_point(data = deseq2_results[deseq2_results$Gene_symbol %in% c("HSD17B2", "CDH3"), ], 
             aes(x = log2FoldChange, y = log10padj), 
             size = 6.5, color = "black", shape = 17) +
  geom_point(data = deseq2_results[deseq2_results$Gene_symbol %in% c("CYBA", "CENPN", "ARY2"), ],
             aes(x = log2FoldChange, y = log10padj), 
  ...
  geom_text_repel(data = deseq2_results[deseq2_results$Gene_symbol %in% "CDH3", ],
  ...
  geom_text_repel(data = deseq2_results[deseq2_results$Gene_symbol %in% "HSD17B2", ],
                  ...
                  nudge_x = 0,
                  size = 8) +
  geom_point(data = deseq2_results[deseq2_results$Gene_symbol %in% c("CDH3", "HSD17B2"), ],
             aes(x = log2FoldChange, y = log10padj), 
             size = 6, color = "black", shape = 17) +
  geom_point(data = deseq2_results[deseq2_results$Gene_symbol %in% c("CDH3", "HSD17B2"), ],
             size = 5, color = "red", shape = 17) +
  geom_text_repel(data = deseq2_results[deseq2_results$Gene_symbol %in% "CENPN", ],
                  aes(x = log2FoldChange, 
                      y = log10padj,
                      label = paste(expression(italic("CENPN"))),
                      color = "black", 
                      shape = "Circle"), 
                  color = "black",
                  box.padding = 0.5,
                  ...
                  size = 8,
                  parse = TRUE) +
  geom_text_repel(data = deseq2_results[deseq2_results$Gene_symbol %in% "CYBA", ],
                  aes(x = log2FoldChange, 
                      y = log10padj,
                      label = paste(expression(italic("CYBA"))),
                      color = "black", 
                      shape = "Circle"), 
                  color = "black",
                  box.padding = 0.5,
                  ...
                  size = 8,
                  parse = TRUE) +
  geom_text_repel(data = deseq2_results[deseq2_results$Gene_symbol %in% "ARY2", ],
                  aes(x = log2FoldChange, 
                      y = log10padj,
                      label = paste(expression(italic("ARY2"))),
                      color = "black", 
                      shape = "Circle"), 
                  color = "black",
                  box.padding = 0.5,
...
ggsave(filename = "main_figures/plots/VolcanoPlot_Hyp_Fae_vs_Ind.svg", plot = p, 
